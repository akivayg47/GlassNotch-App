...
struct Appearance: View {
    ...
    var body: some View {
        Form {
            Section {
                Defaults.Toggle(key: .glassNotchEnabled) {
                    Text("Enable Liquid Glass")
                }

                Picker("Glass style", selection: $glassNotchStyle) {
                    ForEach(GlassNotchStyle.allCases) { style in
                        Text(style.rawValue).tag(style)
                    }
                }
                .disabled(!glassNotchEnabled)

                if glassNotchEnabled {
                    LabeledContent("Opacity", value: "\(Int(glassOpacity * 100))%")
                    Slider(value: $glassOpacity, in: 0.25...1.0, step: 0.01)

                    LabeledContent("Blur", value: "\(Int(glassBlur))")
                    Slider(value: $glassBlur, in: 0...40, step: 1)

                    LabeledContent("Glow", value: "\(Int(glassGlow * 100))%")
                    Slider(value: $glassGlow, in: 0...0.65, step: 0.01)

                    LabeledContent("Saturation") {
                        Text(String(format: "%.2f×", glassSaturation))
                    }
                    Slider(value: $glassSaturation, in: 0.7...1.8, step: 0.01)

                    if glassNotchStyle == .custom {
                        HStack {
                            Text("Custom tint")
                            Spacer()
                            ColorPicker("", selection: Binding(
                                get: { Color(red: glassTintRed, green: glassTintGreen, blue: glassTintBlue) },
                                set: { newColor in
                                    if let components = NSColor(newColor).usingColorSpace(.deviceRGB) {
                                        glassTintRed = components.redComponent
                                        glassTintGreen = components.greenComponent
                                        glassTintBlue = components.blueComponent
                                    }
                                }
                            ))
                            .labelsHidden()
                    ...
                    Button("Reset Glass Settings") {
                        glassNotchStyle = .adaptive
                        glassOpacity = 0.78
                        glassBlur = 22
                        glassGlow = 0.28
                ...
                Text("Choose a Liquid Glass preset or create your own. Changes appear immediately in the notch.")
            }
